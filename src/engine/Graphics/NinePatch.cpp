...
void extractScalePositionsAndContentAreaFromTexture(const sf::Texture* const pTexture, const sf::IntRect textureRectangle, sf::Vector2f& topLeft, sf::Vector2f& bottomRight, sf::Vector2f& contentTopLeft, sf::Vector2f& contentBottomRight)
{
	...
	for (unsigned int x{ 1u }; x < static_cast<unsigned int>(textureRectangle.size.x); ++x)
	{
		if (!foundStart)
		{
			if (image.getPixel(sf::Vector2u(textureRectangle.position.x + static_cast<int>(x), textureRectangle.position.y)) == sf::Color::Black)
			{
				foundStart = true;
				topLeft.x = x - 1.f;
			}
	...
	for (unsigned int y{ 1u }; y < static_cast<unsigned int>(textureRectangle.size.y); ++y)
	{
		if (!foundStart)
		{
			if (image.getPixel(sf::Vector2u(textureRectangle.position.x, textureRectangle.position.y + static_cast<int>(y))) == sf::Color::Black)
			{
				foundStart = true;
				topLeft.y = y - 1.f;
			}
	...
	for (unsigned int x{ 1u }; x < static_cast<unsigned int>(textureRectangle.size.x); ++x)
	{
		if (!foundStart)
		{
			if (image.getPixel(sf::Vector2u(textureRectangle.position.x + static_cast<int>(x), textureRectangle.position.y + static_cast<int>(textureBottomRightPixel.y))) == sf::Color::Black)
			{
				foundStart = true;
	...
	for (unsigned int y{ 1u }; y < static_cast<unsigned int>(textureRectangle.size.y); ++y)
	{
		if (!foundStart)
		{
			if (image.getPixel(sf::Vector2u(textureRectangle.position.x + static_cast<int>(textureBottomRightPixel.x), textureRectangle.position.y + static_cast<int>(y))) == sf::Color::Black)
			{
				foundStart = true;
...
NinePatch::NinePatch()
	: m_primitiveType{ sf::PrimitiveType::TriangleFan }
	, m_vertices(36, sf::Vertex{{ 0.f, 0.f }, sf::Color::White, { 0.f, 0.f }})
	, m_texture{ nullptr }
...
void NinePatch::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (std::size_t offset = 0; offset < m_vertices.size(); offset += 4)
		target.draw(m_vertices.data() + offset, 4, m_primitiveType, states);
}
