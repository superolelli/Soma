...
void NotificationRenderer::Update()
{
	for (auto it = notifications.begin(); it != notifications.end();)
	{
		...
		it->text.move(sf::Vector2f(0.0f, it->up ? -2.0f : 2.0f));
...
{
	Notification newNotification{sf::Text(_font), _duration, up};
	...
	newNotification.text.setPosition(_position - sf::Vector2f(newNotification.text.getLocalBounds().size.x / 2, 0.0));
