...
namespace sfe
{
	...
	void RichText::setString(const sf::String& source)
	{
		...
		for (unsigned int i = 0; i < chunks.size(); ++i)
		{
			...
			{
				t.setPosition(sf::Vector2f(0, t.getPosition().y + font->getLineSpacing(characterSize)));
				++cursor;
			}
	...
	void RichText::center()
	{
		...
		for (int i = 0; i < texts.size(); i++)
		{
			...
			currentLength += texts[i].getLocalBounds().size.x;
			numOfTextInLine++;
		}
		...
		for (int i = 0; i < lineLengths.size(); i++)
		{
			...
			{
				texts[text].setPosition(sf::Vector2f(texts[text].getPosition().x + positionX, texts[text].getPosition().y));
				text++;
			}
