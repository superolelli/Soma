	...
	, m_Buttonstring(_str)
{
	...
	m_Buttontext.setPosition(sf::Vector2f(m_ButtonSprite.GetRect().position.x + (m_ButtonSprite.GetRect().size.x - m_Buttontext.getGlobalBounds().size.x) / 2, m_ButtonSprite.GetRect().position.y + (m_ButtonSprite.GetRect().size.y - m_Buttontext.getGlobalBounds().size.y) / 2));
