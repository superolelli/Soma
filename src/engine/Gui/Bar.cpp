...
void Bar::Render(sf::RenderTarget &_target, bool _withNumbers)
{
	if (m_pValue != nullptr && m_pMaxValue != nullptr)
	{
		...
		if (_withNumbers)
		{
			...
			m_text.setPosition(sf::Vector2f(m_bar.GetRect().size.x / 2 + m_bar.GetRect().position.x - m_text.getGlobalBounds().size.x / 2, m_bar.GetRect().size.y / 2 + m_bar.GetRect().position.y - m_text.getGlobalBounds().size.y / 2 - 2));
			_target.draw(m_text);
		}
	}
}
