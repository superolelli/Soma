...
void CStringInput::Render(sf::RenderTarget &_target)
{
	...
	m_text.setPosition(sf::Vector2f(static_cast<float>(m_xPos - m_text.getGlobalBounds().size.x/2), static_cast<float>(m_yPos)));

	m_line[0].position = sf::Vector2f(static_cast<float>(m_text.getPosition().x + m_text.getGlobalBounds().size.x + 5), static_cast<float>(m_yPos + 5));
	...
	//draw the cursor
	if(m_showCursor)
		_target.draw(m_line, 2, sf::PrimitiveType::Lines);
}
