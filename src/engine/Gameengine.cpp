	...
	, m_simpleRenderLoop(false)
{
	...
	m_RenderTextures[0].resize(sf::VideoMode::getDesktopMode().size);
	m_RenderTextures[1].resize(sf::VideoMode::getDesktopMode().size);
}
...
void CGameEngine::FlushRenderTarget(sf::Shader *_shader)
{
	...
	sf::Sprite sprite(m_RenderTextures[currentRenderTexture].getTexture());
