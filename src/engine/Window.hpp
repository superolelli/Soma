...
using KeyID = sf::Keyboard::Key;
using ButtonID = sf::Mouse::Button;
...
public:
	...
	inline const sf::RenderWindow *GetRenderWindow() const { return m_pWindow; }

	void SetVerticalSyncEnabled(bool _enable) { m_pWindow->setVerticalSyncEnabled(_enable); }

	//The getters
	Keystates const &GetKeystates(KeyID _id) const { return m_keyStates[static_cast<std::size_t>(_id)]; }
	Keystates const &GetButtonstates(ButtonID _id) const { return m_mouseStates[static_cast<std::size_t>(_id)]; }
