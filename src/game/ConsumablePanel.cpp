...
#include "../engine/Patterns/Subject.hpp"
	...
	, itemRowPanel(_engine, new ItemRowPanel(_engine))
{
	...
	itemRowPanel.SetPos(consumablePanel.GetRect().position.x + 49, consumablePanel.GetRect().position.y + 44);
...
InventoryItemWrapper * ConsumablePanel::OnItemFromItemPanelReceived(InventoryItemWrapper * _receivedItem)
{
	if (adventureGroup != nullptr)
	{
		for (int i = 0; i < 4; i++)
		{
			...
			rect.position.x -= engine->GetViewPosition().x;
			if (_receivedItem->GetGlobalBounds().findIntersection(rect))
			{
