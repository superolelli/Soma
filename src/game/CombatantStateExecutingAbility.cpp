...
void CombatantStateExecutingAbility::CalculateCombatantAnimationPositions()
{
	...
	{
		combatantAnimationPositions[context->GetBattlePos()] = sf::Vector2i(context->engine->GetWindowSize().x / 2 - context->GetRect().size.x * COMBATANT_SCALE / 2, 800);
	}
	else if (ability->possibleAims.attackAll)
	{
		for (int i = 0; i < context->allCombatants->size(); i++)
			combatantAnimationPositions[context->allCombatants->at(i)->GetBattlePos()] = sf::Vector2i(context->allCombatants->at(i)->GetRect().position.x - context->engine->GetViewPosition().x, 800);
	}
	...
	else
	{
		...
		combatantAnimationPositions[context->GetBattlePos()] = sf::Vector2i(attackerPosition - context->GetRect().size.x * COMBATANT_SCALE / 2, 800);
...
int CombatantStateExecutingAbility::CalculateSelectedTargetsTotalWidth(int _padding)
{
	...
	for (auto* c : context->selectedTargets)
		width += c->GetRect().size.x * COMBATANT_SCALE + _padding;
	width -= _padding;
...
void CombatantStateExecutingAbility::CalculateSelectedTargetsPositions(int _startingPosition, int _padding)
{
	...
	for (auto* c : context->selectedTargets)
	{
		...
		xPos += c->GetRect().size.x * COMBATANT_SCALE + _padding;
	}
}
...
void CombatantStateExecutingAbility::ApplyAbilityEffect()
{
	...
	if (isCriticalHit)
	{
		context->notificationRenderer->AddNotification("Kritisch!", g_pFonts->f_blackwoodCastle, sf::Vector2f(context->GetRect().position.x + context->GetRect().size.x / 2.0f, context->GetRect().position.y - 40), 1.0f);
