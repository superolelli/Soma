...
void ShopPanel::Update()
{
	...
	if (g_pGameStatus->GetNumberOfShopSlots() < 5 && shopSlotLock.GetRect().contains(engine->GetMousePos()))
	{
		...
		{
			sf::String description = sf::String("Den Slot f�r ") + std::to_string(SLOT_UNLOCK_COST) + " Karten freischalten?";
	...
	if (g_pGameStatus->GetShopDicePrice() > MINIMUM_DICE_PRICE && dicePriceLock.GetRect().contains(engine->GetMousePos()))
	{
		...
		{
			sf::String description = sf::String("Den W�rfelpreis f�r ") + std::to_string(DICE_PRICE_REDUCE_COST) + "\nKarten um " + std::to_string(DICE_PRICE_REDUCE_STEP) + " reduzieren?";
		...
		for (int i = 0; i < 16; i++)
		{
			if (items[i] != nullptr && items[i]->Contains(engine->GetMousePos()))
			{
				...
				else
				{
					...
					selectedItemFrame.SetPos(items[i]->GetGlobalBounds().position.x - 7, items[i]->GetGlobalBounds().position.y - 7);
...
void ShopPanel::Render()
{
	...
	for (int i = 0; i < 16; i++)
	{
		if (items[i] != nullptr)
		{
			...
			priceSign.SetPos(items[i]->GetGlobalBounds().position.x + 18, items[i]->GetGlobalBounds().position.y + 90);
	...
	{
		shopSlotLock.SetPos(i * 116 + shopPanel.GetGlobalRect().position.x + 23, shopPanel.GetGlobalRect().position.y + 105);
	...
	if (g_pGameStatus->GetShopDicePrice() > MINIMUM_DICE_PRICE)
	{
		...
		priceSign.SetPos(dicePriceLock.GetRect().position.x + 5, dicePriceLock.GetRect().position.y + 20);
		priceSign.SetTextPosCentered(0);
		priceSign.Render(engine->GetRenderTarget());
...
void ShopPanel::SetPos(int _x, int _y)
{
	...
	title.setPosition(sf::Vector2f(shopPanel.GetGlobalRect().position.x + 140, shopPanel.GetGlobalRect().position.y + 25));

	for (int i = 0; i < 5; i++)
	{
		if (items[i] != nullptr)
		{
			int xPos = i * 116 + shopPanel.GetGlobalRect().position.x + 33;
			int yPos = shopPanel.GetGlobalRect().position.y + 114;
	...
	for (int i = 5; i < 15; i++)
	{
		if (items[i] != nullptr)
		{
			int xPos = ((i - 5) % 5) * 116 + shopPanel.GetGlobalRect().position.x + 33;
			int yPos = shopPanel.GetGlobalRect().position.y + 297 + 144 * ((i - 5) / 5);
			items[i]->SetPos(xPos, yPos);
		}
	}

	int xPos = shopPanel.GetGlobalRect().position.x + 671;
	int yPos = shopPanel.GetGlobalRect().position.y + 114;
	items[15]->SetPos(xPos, yPos);

	dicePriceLock.SetPos(shopPanel.GetGlobalRect().position.x + 685, shopPanel.GetGlobalRect().position.y + 255);
}
