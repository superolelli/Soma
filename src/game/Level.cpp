...
{
	battle = false;
	currentRoomNumber = (_playerPos - rooms[0]->background.GetRect().size.x / 2) / rooms[0]->background.GetRect().size.x;
...
{
	return _playerPos >= rooms.size() * rooms[0]->background.GetRect().size.x - rooms[0]->background.GetRect().size.x / 2;
}
...
{
	//Render fourth Layer
	g_pSpritePool->backgroundFourthLayer[levelType]->SetPos(_viewX - (_viewX / 9) % g_pSpritePool->backgroundFourthLayer[levelType]->GetRect().size.x, 0);
	g_pSpritePool->backgroundFourthLayer[levelType]->Render(_target);
	g_pSpritePool->backgroundFourthLayer[levelType]->SetPos(g_pSpritePool->backgroundFourthLayer[levelType]->GetRect().position.x + g_pSpritePool->backgroundFourthLayer[levelType]->GetRect().size.x, 0);
	g_pSpritePool->backgroundFourthLayer[levelType]->Render(_target);

	//Render third Layer
	int newX = _viewX - (_viewX / 6) % g_pSpritePool->backgroundThirdLayers[levelType][currentThirdLayer1].GetRect().size.x;
	if (newX >= g_pSpritePool->backgroundThirdLayers[levelType][currentThirdLayer1].GetRect().position.x + g_pSpritePool->backgroundThirdLayers[levelType][currentThirdLayer1].GetRect().size.x)
	{
		currentThirdLayer1 = currentThirdLayer2;
		currentThirdLayer2 = rand() % g_pSpritePool->backgroundThirdLayers[levelType].size();
	}

	g_pSpritePool->backgroundThirdLayers[levelType][currentThirdLayer2].SetPos(newX + g_pSpritePool->backgroundThirdLayers[levelType][currentThirdLayer1].GetRect().size.x, 0);
	...
	//Render second Layer
	newX = _viewX - (_viewX / 3) % g_pSpritePool->backgroundSecondLayers[levelType][currentSecondLayer1].GetRect().size.x;
	if (newX >= g_pSpritePool->backgroundSecondLayers[levelType][currentSecondLayer1].GetRect().position.x + g_pSpritePool->backgroundSecondLayers[levelType][currentSecondLayer1].GetRect().size.x)
	{
		currentSecondLayer1 = currentSecondLayer2;
		currentSecondLayer2 = rand() % g_pSpritePool->backgroundSecondLayers[levelType].size();
	}

	g_pSpritePool->backgroundSecondLayers[levelType][currentSecondLayer2].SetPos(newX + g_pSpritePool->backgroundSecondLayers[levelType][currentSecondLayer1].GetRect().size.x, 0);
