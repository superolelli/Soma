	...
	, isOpen(true)
{
	...
	SetPos(engine->GetWindowSize().x / 2 - lootablePanel.GetGlobalRect().size.x / 2, 200);
...
void LootableDialog::SetTitle(std::string const &_str)
{
	...
	lootableNameText.setPosition(sf::Vector2f(lootablePanel.GetGlobalRect().position.x + 467 - lootableNameText.getGlobalBounds().size.x / 2, lootablePanel.GetGlobalRect().position.y + 15));
}

void LootableDialog::SetPos(int _x, int _y)
{
	lootablePanel.SetPos(_x, _y);
	buttonClose.SetPos(lootablePanel.GetGlobalRect().position.x + 522, lootablePanel.GetGlobalRect().position.y + 65);
	buttonTakeAll.SetPos(lootablePanel.GetGlobalRect().position.x + 423, lootablePanel.GetGlobalRect().position.y + 342);
	lootableNameText.setPosition(sf::Vector2f(lootablePanel.GetGlobalRect().position.x + 467 - lootableNameText.getGlobalBounds().size.x / 2, lootablePanel.GetGlobalRect().position.y + 15));
}
...
void LootableDialog::AddItem(Item _item)
{
	...
	int xPos = (freeSlot % 3) * 116 + lootablePanel.GetGlobalRect().position.x + 46;
	int yPos = (freeSlot / 3) * 116 + lootablePanel.GetGlobalRect().position.y + 92;
