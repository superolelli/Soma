...
void CombatantAttributesPanel::InitDescriptionTexts()
{
	...
	statsText.setPosition(sf::Vector2f(0.0f, 0.0f));
}

void CombatantAttributesPanel::InitValueTexts()
{
	int yPos = statsText.getGlobalBounds().position.y;
	for (auto &text : statValueText)
	{
		...
		text.setPosition(sf::Vector2f(statsText.getGlobalBounds().position.x + statsText.getGlobalBounds().size.x + 10.0f, yPos));
		yPos += 20;
	}
}
...
{
	statsText.setPosition(sf::Vector2f(_x + 10.0f, _y + 7.0f));
...
void CombatantAttributesPanel::Update(CombatantStatus &_status)
{
	...
	auto backgroundWidth = statValueText[0].getGlobalBounds().position.x - statsText.getGlobalBounds().position.x + 50.0f;
	background.setSize(sf::Vector2f(backgroundWidth, statsText.getLocalBounds().size.y + 20.0f));
