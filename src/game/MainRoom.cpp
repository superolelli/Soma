	...
	, xMovement(0.0f)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		xPos += background[i].GetGlobalRect().size.x;
	}
	...
	view = sf::View(sf::FloatRect({0.0f, 0.0f}, {static_cast<float>(_engine->GetWindowSize().x), static_cast<float>(_engine->GetWindowSize().y)}));
...
void MainRoom::InitPlayers()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		players[i]->setSoundPosition(playerHitbox[i].position.x + playerHitbox[i].size.x / 2, playerHitbox[i].position.y + playerHitbox[i].size.y / 2);
	}
}
...
void MainRoom::UpdatePlayerHitboxes()
{
	for (int i = 0; i < 4; i++)
	{
		...
		playerHitbox[i].position.x = hitboxObj->getPosition().x;
		playerHitbox[i].position.y = hitboxObj->getPosition().y;
		playerHitbox[i].size.x = hitboxObj->getSize().x  * hitboxObj->getScale().x;
		playerHitbox[i].size.y = hitboxObj->getSize().y * hitboxObj->getScale().y;
	}
}
...
void MainRoom::Update()
{
	...
	if (!exitDialog.IsOpen()) {
		...
		if (view.getCenter().x - m_pGameEngine->GetWindowSize().x / 2 + xMovement >= 0
			&& view.getCenter().x + m_pGameEngine->GetWindowSize().x / 2 + xMovement < background[0].GetGlobalRect().size.x * 4) {
			view.move(sf::Vector2f(xMovement, 0));
...
{
	sf::IntRect viewRect = {
		{static_cast<int>(view.getCenter().x - view.getSize().x / 2), static_cast<int>(view.getCenter().y - view.getSize().y / 2)},
		{static_cast<int>(view.getSize().x), static_cast<int>(view.getSize().y)}};

	for (auto &b : background) {
		if (b.GetGlobalRect().findIntersection(viewRect))
			b.Render(m_pGameEngine->GetRenderTarget());
	}

	for (auto &d : doors) {
		if (d.GetGlobalRect().findIntersection(viewRect))
			d.Render(m_pGameEngine->GetRenderTarget());
	}

	if (roots.GetGlobalRect().findIntersection(viewRect))
		roots.Render(m_pGameEngine->GetRenderTarget());

	for (auto &s : signs) {
		if (s.GetRect().findIntersection(viewRect))
