...
{
	playerContext->notificationRenderer->AddNotification("Verwirrt!", g_pFonts->f_blackwoodCastle, sf::Vector2f(playerContext->GetRect().position.x + playerContext->GetRect().size.x / 2.0f, playerContext->GetRect().position.y - 20.0f), 1.0f);
...
void PlayerStatePrepareAbility::HandleStupidness()
{
	...
	if (isStupid == 0)
	{
		playerContext->notificationRenderer->AddNotification("Dummheit", g_pFonts->f_blackwoodCastle, sf::Vector2f(playerContext->GetRect().position.x + playerContext->GetRect().size.x / 2.0f, playerContext->GetRect().position.y - 20.0f), 1.0f);
		notificationWaitingTime = 1.5;
