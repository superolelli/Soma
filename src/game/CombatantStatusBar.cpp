...
{
	healthBar.SetPos(_combatantRect.position.x + _combatantRect.size.x / 2 - healthBar.GetRect().size.x / 2, _combatantRect.position.y + _combatantRect.size.y + 30);
...
void CombatantStatusBar::Render()
{
	...
	if (isTurnPending)
	{
		g_pSpritePool->turnPendingMarker->SetPos(healthBar.GetRect().position.x + healthBar.GetRect().size.x / 2 - g_pSpritePool->turnPendingMarker->GetRect().size.x / 2, healthBar.GetRect().position.y - g_pSpritePool->turnPendingMarker->GetRect().size.y - 2);
...
{
	int x = healthBar.GetRect().position.x;
	int y = healthBar.GetRect().position.y + healthBar.GetRect().size.y;
...
void CombatantStatusBar::RenderStatusSymbolsTooltips()
{
	...
	if (type != CombatantStatus::Type::number_of_states) {
		...
		tooltip.ShowTooltip(engine->GetRenderTarget(), healthBar.GetRect().position.x, healthBar.GetRect().position.y - 30);
	}
}