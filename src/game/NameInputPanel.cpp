...
void NameInputPanel::SetPos(int _x, int _y)
{
    ...
    nameInput.SetPos(_x + backgroundSprite.GetRect().size.x/2, _y + 15);
    finishedButton.SetPos(_x + backgroundSprite.GetRect().size.x / 2 - finishedButton.GetRect().size.x / 2, _y + backgroundSprite.GetRect().size.y + 10);
}
