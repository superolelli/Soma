	...
	, equipmentPanel(_engine)
{
	...
	scrollableItemPanel.SetPos(inventoryPanel.GetRect().position.x + 843, inventoryPanel.GetRect().position.y + 212);

	buttonSortNames.SetPos(scrollableItemPanel.GetRect().position.x + scrollableItemPanel.GetRect().size.x + 11, scrollableItemPanel.GetRect().position.y - 14);
	buttonSortNames.SetCallback([]() {g_pSounds->PlaySound(soundID::CLICK); });

	buttonSortColors.SetPos(scrollableItemPanel.GetRect().position.x + scrollableItemPanel.GetRect().size.x + 11, buttonSortNames.GetRect().position.y + buttonSortNames.GetRect().size.y - 2);
	buttonSortColors.SetCallback([]() {g_pSounds->PlaySound(soundID::CLICK); });

	buttonNext.SetPos(inventoryPanel.GetGlobalRect().position.x + 217, inventoryPanel.GetGlobalRect().position.y + 66);
	buttonNext.SetCallback([]() {g_pSounds->PlaySound(soundID::CLICK); });

	buttonPrevious.SetPos(inventoryPanel.GetGlobalRect().position.x + 15, inventoryPanel.GetGlobalRect().position.y + 66);
	buttonPrevious.SetCallback([]() {g_pSounds->PlaySound(soundID::CLICK); });

	buttonClose.SetPos(inventoryPanel.GetGlobalRect().position.x + 1475, inventoryPanel.GetGlobalRect().position.y + 66);
	...
	currentPlayerName.setPosition(sf::Vector2f(inventoryPanel.GetGlobalRect().position.x + 52 + (162 - currentPlayerName.getLocalBounds().size.x) / 2, inventoryPanel.GetGlobalRect().position.y + 55));
	...
	panelTitle.setPosition(sf::Vector2f(inventoryPanel.GetGlobalRect().position.x + 850, inventoryPanel.GetGlobalRect().position.y + 70));

	equipmentPanel.SetPos(inventoryPanel.GetRect().position.x + 309, inventoryPanel.GetRect().position.y + 287);
	...
	playerAttributesText.SetPos(inventoryPanel.GetRect().position.x + 65, inventoryPanel.GetRect().position.y + 330);
...
{
	if (_receivedItem->GetGlobalBounds().findIntersection(scrollableItemPanel.GetRect()))
	{
		g_pSounds->PlaySound(soundID::INVENTORY_DROP);
