...
void ScrollableItemPanelDragAndDrop::HandleStartedDrag()
{
	...
	int x = engine->GetMousePos().x - (scrollableItemPanel.GetGlobalRect().position.x + 11);
	int y = engine->GetMousePos().y - (scrollableItemPanel.GetGlobalRect().position.y + 11);
	...
	if (itemPointedTo >= 0 && items.size() > itemPointedTo && items[itemPointedTo] != nullptr)
	{
		...
		{
			currentDraggedItemOldX = items[itemPointedTo]->GetGlobalBounds().position.x;
			currentDraggedItemOldY = items[itemPointedTo]->GetGlobalBounds().position.y;
