...
void ScrollableItemPanel::SetPos(int _x, int _y)
{
	...
	scrollbar.SetPos(itemPanel.GetGlobalRect().position.x + 600, itemPanel.GetGlobalRect().position.y + 12);
}
...
{
	int yPos = itemPanel.GetGlobalRect().position.y + 11;
	int currentIndex = 0;

	for (int y = 0; y < 5; y++)
	{
		int xPos = itemPanel.GetGlobalRect().position.x + 11;
		for (int x = 0; x < 5; x++)
		{
...
void ScrollableItemPanel::AddItem(Item _item)
{
	...
	int xPos = (freeSlot % 5) * 116 + itemPanel.GetGlobalRect().position.x + 11;
	int yPos = (freeSlot / 5) * 116 + itemPanel.GetGlobalRect().position.y + 11 - currentUpperRow * 116;
