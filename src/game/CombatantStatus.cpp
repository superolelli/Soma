...
void CombatantStatus::ExecuteStatusChanges()
{
	...
	else if (!sleepChecked)
	{
		...
		if (sleeping == true)
		{
			...
			notificationRenderer->AddNotification("Aufgewacht!", g_pFonts->f_blackwoodCastle, sf::Vector2f(combatant->GetRect().position.x + combatant->GetRect().size.x / 2.0f, combatant->GetRect().position.y - 20.0f), 1.0f);
		}
	}
}
...
void CombatantStatus::HandleDynamite()
{
	if (dynamite)
	{
		if (rand() % 5 == 0)
		{
			...
			notificationRenderer->AddNotification("Explodiert!", g_pFonts->f_blackwoodCastle, sf::Vector2f(combatant->GetRect().position.x + combatant->GetRect().size.x / 2.0f, combatant->GetRect().position.y - 40.0f), 1.0f);
...
void CombatantStatus::LooseHealth(int _damage, bool _critical, bool _useArmour)
{
	...
	auto notificationPos = sf::Vector2f(combatant->GetRect().position.x + combatant->GetRect().size.x / 2.0f, combatant->GetRect().position.y);
...
void CombatantStatus::RemoveAllBuffs()
{
	...
	notificationRenderer->AddNotification("Buffs entfernt!", g_pFonts->f_blackwoodCastle, sf::Vector2f(combatant->GetRect().position.x + combatant->GetRect().size.x / 2.0f, combatant->GetRect().position.y - 40.0f), 1.0f);
}
...
void CombatantStatus::RemoveAllDebuffs()
{
	...
	notificationRenderer->AddNotification("Debuffs entfernt!", g_pFonts->f_blackwoodCastle, sf::Vector2f(combatant->GetRect().position.x - combatant->GetRect().size.x / 2.0f, combatant->GetRect().position.y - 40.0f), 1.0f);
}
