	...
	, currentlyActive(false)
{
	...
	resultTexture.resize({static_cast<unsigned int>(connection.getGlobalBounds().size.x), static_cast<unsigned int>(connection.getGlobalBounds().size.y)});
	resultTexture.clear();
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, connection.getGlobalBounds().size.x, connection.getGlobalBounds().size.y, 0, GL_RGBA, GL_FLOAT, NULL);
	glBindTexture(GL_TEXTURE_2D, 0);

	activationShader.loadFromFile("Data/Shader/equipment_connections_activate.frag", sf::Shader::Type::Fragment);
	activationShader.setUniform("u_resolution", sf::Glsl::Vec2(connection.getGlobalBounds().size.x, connection.getGlobalBounds().size.y));
	...
	deactivationShader.loadFromFile("Data/Shader/equipment_connections_deactivate.frag", sf::Shader::Type::Fragment);
	deactivationShader.setUniform("u_resolution", sf::Glsl::Vec2(connection.getGlobalBounds().size.x, connection.getGlobalBounds().size.y));
...
void EquipmentConnections::Update()
{
	...
	if (performingDeactivation)
	{
		...
		connection.setPosition(sf::Vector2f(0, 0));
