...
void AbilityTooltip::ShowPossibleTargets(sf::RenderTarget &_target, int _x, int _y, bool _showStacked)
{
	...
	targetsBackground.setPosition(sf::Vector2f(_x, _y));
	...
	for (int i = 0; i < 4; i++)
	{
		possibleTargets[i].SetPos(targetsBackground.getPosition().x + targetX, targetsBackground.getPosition().y - possibleTargets[i].GetGlobalRect().size.y + 37);
		targetX += possibleTargets[i].GetRect().size.x;
	}
	...
	{
		possibleTargets[i].SetPos(targetsBackground.getPosition().x + targetX, targetsBackground.getPosition().y - possibleTargets[i].GetGlobalRect().size.y + 37);
		targetX += possibleTargets[i].GetRect().size.x + 1;
	}
	...
	if(_showStacked)
		targetsInformationText.setPosition(sf::Vector2f(targetsBackground.getPosition().x + 10, targetsBackground.getPosition().y + 43));
	else
		targetsInformationText.setPosition(sf::Vector2f(targetsBackground.getPosition().x + 10, targetsBackground.getPosition().y + 10));
