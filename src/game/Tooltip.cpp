...
void Tooltip::ShowTooltip(sf::RenderTarget &_target, int _x, int _y)
{
	...
	tooltipBackground.setSize(sf::Vector2f(backgroundRect.size.x + 20.0f, backgroundRect.size.y + 20.0f));

	if (showTooltipAboveY)
		tooltipBackground.setPosition(sf::Vector2f(_x, _y - tooltipText.getLocalBounds().size.y));
	else
		tooltipBackground.setPosition(sf::Vector2f(_x, _y));

	tooltipText.setPosition(sf::Vector2f(tooltipBackground.getGlobalBounds().position.x + 10, tooltipBackground.getGlobalBounds().position.y + 7));
