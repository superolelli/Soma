...
void Combatant::ReloadHitbox()
{
	...
	{
		hitbox.position.x = hitboxObj->getPosition().x;
		hitbox.position.y = hitboxObj->getPosition().y;
		hitbox.size.x = hitboxObj->getSize().x  * hitboxObj->getScale().x;
		hitbox.size.y = hitboxObj->getSize().y * hitboxObj->getScale().y;
	}
}
...
void Combatant::RenderShadow()
{
	...
	shadow.setRadius(GetRect().size.x / 2 + 10);
	shadow.setPointCount(60);
	shadow.scale(sf::Vector2f(1.0, 0.2));
	...
	shadow.setPosition(sf::Vector2f(pointObj->getPosition().x - shadow.getLocalBounds().size.x / 2, pointObj->getPosition().y- shadow.getGlobalBounds().size.y / 2));
	engine->GetRenderTarget().draw(shadow);
}

void Combatant::RenderAbilityTargetMarker()
{
	int xPos = GetRect().position.x + (GetRect().size.x - g_pSpritePool->abilityTargetMarker->GetRect().size.x) / 2;
	int yPos = statusBar.GetRect().position.y + statusBar.GetRect().size.y + 5;
...
void Combatant::RenderTurnMarker()
{
	...
	int xPos = GetRect().position.x + (GetRect().size.x - g_pSpritePool->turnMarker->GetRect().size.x) / 2;
	int yPos = statusBar.GetRect().position.y + statusBar.GetRect().size.y + 5;
...
void Combatant::StartDodgingAnimation(int _xPos, int _yPos)
{
	...
	SetAnimation("attacked", ABILITY_ANIMATION_SPEED);
	ReloadHitbox();
	notificationRenderer->AddNotification("Ausgewichen!", g_pFonts->f_blackwoodCastle, sf::Vector2f(GetRect().position.x + GetRect().size.x / 2.0f, GetRect().position.y), 1.0f);
...
void Combatant::ApplyAbilityEffect(Combatant * _attacker, AbilityEffect & _effect, float _additionalDamageFactor)
{
	...
	if (isCriticalHit)
	{
		notificationRenderer->AddNotification("Kritisch!", g_pFonts->f_blackwoodCastle, sf::Vector2f(GetRect().position.x + GetRect().size.x / 2.0f, GetRect().position.y - 40), 1.0f);
...
{
	notificationRenderer->AddNotification("Widerstanden!", g_pFonts->f_blackwoodCastle, sf::Vector2f(GetRect().position.x + GetRect().size.x / 2.0f, GetRect().position.y - 30), 1.0f);
