...
sf::FloatRect CombatantResistancesPanel::GetRect()
{
	...
	{
		if (t.getGlobalBounds().position.x + t.getGlobalBounds().size.x > rect.position.x + rect.size.x)
			rect.size.x = t.getGlobalBounds().position.x + t.getGlobalBounds().size.x - rect.position.x;
	}
...
{
	title.setPosition(sf::Vector2f(_x, _y));

	int yPos = title.getGlobalBounds().position.y + 30;
	for (auto& text : resistancesText)
	{
		text.setPosition(sf::Vector2f(_x + 70, yPos));
		yPos += 25;
	}
}
...
void CombatantResistancesPanel::Render(sf::RenderTarget& _target)
{
	...
	g_pSpritePool->sleeping->SetPos(title.getGlobalBounds().position.x + 40, resistancesText[0].getGlobalBounds().position.y - 2);
	g_pSpritePool->sleeping->Render(_target);

	g_pSpritePool->confused->SetPos(title.getGlobalBounds().position.x + 40, resistancesText[1].getGlobalBounds().position.y - 2);
	g_pSpritePool->confused->Render(_target);

	g_pSpritePool->debuff->SetPos(title.getGlobalBounds().position.x + 40, resistancesText[2].getGlobalBounds().position.y - 2);
	g_pSpritePool->debuff->Render(_target);

	g_pSpritePool->decay->SetPos(title.getGlobalBounds().position.x + 40, resistancesText[3].getGlobalBounds().position.y - 2);
