...
void EquipmentPanel::HandleDragStarted()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			currentDraggedItemOldX = items[currentPlayer][i]->GetGlobalBounds().position.x;
			currentDraggedItemOldY = items[currentPlayer][i]->GetGlobalBounds().position.y;
...
InventoryItemWrapper * EquipmentPanel::PlaceItem(InventoryItemWrapper *_item)
{
	for (int i = 0; i < 4; i++)
	{
		if (_item->GetGlobalBounds().findIntersection(equipmentField[i].GetGlobalRect()))
		{
			auto oldItem = items[currentPlayer][i];
			items[currentPlayer][i] = _item;
			items[currentPlayer][i]->SetPos(equipmentField[i].GetGlobalRect().position.x + 10, equipmentField[i].GetGlobalRect().position.y + 10);
...
bool EquipmentPanel::CanBePlaced(CSprite & _itemSprite)
{
	for (auto &e : equipmentField)
	{
		if (_itemSprite.GetGlobalRect().findIntersection(e.GetGlobalRect()))
			return true;
	}
...
void EquipmentPanel::AddItem(Item& _item, int player, int slot)
{
	...
	newItem->SetPos(equipmentField[slot].GetGlobalRect().position.x + 10, equipmentField[slot].GetGlobalRect().position.y + 10);
